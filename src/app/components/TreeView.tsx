...
import { classificationBadgeClass, classificationTextClass, sensitivityTextClass, KeyBadge, RecordTypeIcon } from '../lib/badges';
  ...
  return (
    <div>
      <div className="flex items-center gap-0 pl-5 pr-5 py-2.5 hover:bg-purple-50/30 transition-colors group">
        {/* Expand toggle — chevron + entity identity (click matches the chevron's accordion affordance) */}
        <button
          onClick={() => setOpen(v => !v)}
          aria-expanded={open}
          className="flex items-center min-w-0 text-left"
        >
          <span className="flex items-center justify-center w-5 h-5 mr-2 flex-shrink-0 text-gray-400 group-hover:text-purple-600 transition-colors">
            {open ? <ChevronDown className="w-3.5 h-3.5" /> : <ChevronRight className="w-3.5 h-3.5" />}
          </span>

          <RecordTypeIcon type="entity" size="sm" className="mr-2.5" />
        ...
        {/* Inspect — sits right next to the identity so the affordance is obvious, labelled so it's
            clearly a control (and distinct from the expand gesture). */}
        <button
          onClick={() => onEntityClick(entity, subjectArea)}
          title="View full metadata"
          aria-label={`View full metadata for ${entity.name}`}
          className="ml-2.5 flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-medium text-gray-500 hover:text-purple-700 hover:bg-purple-100/70 transition-colors flex-shrink-0"
        >
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 min-w-0">
        <RecordTypeIcon type="dataitem" size="xs" boxless />
